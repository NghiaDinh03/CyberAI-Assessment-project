...
## 8. 📁 Multi-Label Evidence-to-Control Mapping Algorithm
...
Where:
- $\alpha = 0.4$, $\beta = 0.4$, $\gamma = 0.2$ are component weights ($\alpha + \beta + \gamma = 1.0$).
- $\text{Match}_{\text{filename}}$: Evaluates 80+ pre-compiled regular expressions and keyword patterns against filenames.
- $\text{Match}_{\text{content}}$: Measures technical keyword density in OCR-extracted text chunks.
- $\text{QualityScore}(E)$: Weights document freshness, structural completeness, and byte length.

---

## 9. 🧩 Control-Aware Chunking & Token Budget Optimizer

Source: [`backend/services/controls_catalog.py`](../../backend/services/controls_catalog.py), [`backend/services/chat_service.py`](../../backend/services/chat_service.py)

### 9.1 Context Limitation on Quantized Edge Models
Local quantized LLMs on CPU (e.g., `gemma4:latest`) operate with an optimal context window of 4K–16K tokens. Feeding 93 ISO 27001 controls with attached evidence in a single pass degrades attention and exhausts memory.

### 9.2 Co-occurrence Partitioning Strategy
The catalog is partitioned into $\approx 16$ logical groups $\mathcal{G} = \{G_1, G_2, \dots, G_m\}$ where each sub-group contains $k \in [5, 8]$ co-occurring controls:

$$\bigcup_{j=1}^{m} G_j = \mathcal{C}, \quad |G_j| \in [5, 8]$$

Inference is executed iteratively per partition, validating intermediate JSON structures and merging verdicts into the final assessment repository.

---

## 10. 🔧 Self-Healing JSON AST Parser

Source: [`backend/utils/json_repair.py`](../../backend/utils/json_repair.py)

### 10.1 Syntactic Vulnerabilities in Quantized Inference
Quantized 4-bit edge models occasionally emit malformed JSON tokens near output truncation limits (missing closing brackets `]}`, trailing commas, or Python-style single quotes `'`).

### 10.2 4-Stage Syntax Recovery Pipeline:
1. **Regex Normalization:** Converts Python literals (`True`, `False`, `None`) into JSON compliant primitives (`true`, `false`, `null`).
2. **Quote Normalization:** Sanitizes unbalanced single quotes wrapping keys and string values into RFC 8259 double quotes.
3. **Trailing Comma Stripping:** Prunes invalid trailing commas preceding closing delimiters `,\s*[\}\]]`.
4. **AST Stack Auto-Completion:** Uses a bracket balancing stack to track unclosed `{` and `[` tokens, automatically appending necessary termination brackets to restore AST validity.
