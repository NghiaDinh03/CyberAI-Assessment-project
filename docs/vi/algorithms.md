...
## 7. ☁️ Chuỗi dự phòng Cloud LLM (Cloud LLM Fallback Chain - ĐÃ VÔ HIỆU HÓA)

> ⚠️ **Lưu ý bảo mật quan trọng:** Trong phiên bản thực tế hiện tại, để tuân thủ nghiêm ngặt các quy định về bảo mật thông tin và bảo vệ dữ liệu minh chứng nhạy cảm (On-Premise), toàn bộ kết nối Cloud LLM và Fallback Chain trên đám mây đã được **vô hiệu hóa hoàn toàn** trong mã nguồn backend (`CloudLLMService.is_cloud_available()` luôn trả về `False`). Hệ thống hoạt động 100% Offline, thực hiện suy luận cục bộ kép qua Ollama và LocalAI. Các thuật toán mô tả dưới đây đại diện cho thiết kế lý thuyết hoặc cấu hình thử nghiệm trong môi trường Sandbox, không chạy thực tế trên sản phẩm.
...
## 8. 📁 Thuật toán Ánh xạ Bằng chứng Đa nhãn (Multi-Label Evidence-to-Control Mapping)
...
$$\text{Confidence}(E, C_i) = \alpha \cdot \text{Match}_{\text{filename}}(E, K_{C_i}) + \beta \cdot \text{Match}_{\text{content}}(E, K_{C_i}) + \gamma \cdot \text{QualityScore}(E)$$

Trong đó:
- $\alpha = 0.4$, $\beta = 0.4$, $\gamma = 0.2$ là các trọng số thành phần ($\alpha + \beta + \gamma = 1.0$).
- $\text{Match}_{\text{filename}}$: Khớp biểu thức chính quy (Regex) và từ khóa trên tên tệp (80+ patterns định sẵn).
- $\text{Match}_{\text{content}}$: Tần suất xuất hiện của từ khóa kỹ thuật chuyên ngành trong nội dung văn bản (đã trích xuất qua Tesseract OCR/parser).
- $\text{QualityScore}(E)$: Đánh giá độ tươi mới (Recency), kích thước và độ chi tiết của dữ liệu bằng chứng.

---

## 9. 🧩 Thuật toán Phân cụm Kiểm soát & Tối ưu hóa Ngân sách Token (Control-Aware Chunking)

Nguồn: [`backend/services/controls_catalog.py`](../../backend/services/controls_catalog.py), [`backend/services/chat_service.py`](../../backend/services/chat_service.py)

### 9.1 Bài toán Giới hạn Ngữ cảnh (Context Window Limit)
Mô hình cục bộ lượng tử hóa chạy trên CPU (như `gemma4:latest`) có giới hạn hiệu năng suy luận tối ưu tại 4K – 16K tokens. Nếu đưa toàn bộ 93 controls ISO 27001 cùng văn bản bằng chứng vào 1 prompt đơn lẻ, mô hình sẽ bị tràn bộ nhớ hoặc suy giảm độ chính xác nghiêm trọng.

### 9.2 Chiến lược Phân cụm Đồng xuất hiện (Co-occurrence Clustering)
Thuật toán phân tách 93 controls thành $\approx 16$ nhóm logic nhỏ, mỗi nhóm gồm $k \in [5, 8]$ controls có cùng miền kỹ thuật:

$$\mathcal{G} = \{G_1, G_2, \dots, G_m\}, \quad |G_j| \in [5, 8], \quad \bigcup_{j=1}^{m} G_j = \mathcal{C}$$

Mỗi chu trình đánh giá thực hiện suy luận cục bộ độc lập cho từng nhóm $G_j$, sau đó tổng hợp toàn bộ mảng kết quả $\bigcup \text{Verdicts}(G_j)$ vào cơ sở dữ liệu SQLite và bước sinh báo cáo tổng hợp.

---

## 10. 🔧 Cơ chế Tự Phục Hồi Cú Pháp JSON AST (Self-Healing JSON Orchestrator)

Nguồn: [`backend/utils/json_repair.py`](../../backend/utils/json_repair.py)

### 10.1 Hiện tượng Lỗi Cú pháp ở Local Quantized Models
Mô hình mã nguồn mở lượng tử hóa 4-bit chạy trên CPU thường gặp các lỗi cú pháp JSON nhẹ khi kết thúc phản hồi (thiếu dấu đóng `]}` do cắt ngắn max_tokens, xuất hiện dấu phẩy thừa `trailing commas`, sử dụng dấu nháy đơn `'` thay cho `"`).

### 10.2 Pipeline Vá Cú pháp 4 Giai đoạn:
1. **Regex Normalization:** Chuyển đổi hằng số Python (`True`, `False`, `None`) thành chuẩn JSON (`true`, `false`, `null`).
2. **Quote Normalization:** Chuyển đổi nháy đơn bọc chuỗi và keys thành nháy kép chuẩn RFC 8259.
3. **Trailing Comma Stripping:** Loại bỏ toàn bộ dấu phẩy đứng trước dấu đóng ngoặc `,\s*[\}\]]`.
4. **AST Stack Auto-Completion:** Sử dụng ngăn xếp (Stack) duyệt qua chuỗi để đếm các cặp ngoặc `{`, `[` chưa được đóng và tự động thêm các ký tự đóng tương ứng vào cuối chuỗi.
