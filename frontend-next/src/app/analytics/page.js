...
import { BarChart2, BookOpen, FlaskConical, Trash2, RefreshCw, Plus, Search, Filter } from 'lucide-react'
...
export default function AnalyticsPage() {
    ...
    const ollamaReady = !!(aiStatus?.ollama?.status === 'healthy' || ollamaModels.length > 0 || backendReady)
    const ollamaModelName = ollamaModels.length > 0 ? ollamaModels[0].name || ollamaModels[0].id || 'Gemma 4' : 'Gemma 4'
    const chromaReady = backendReady
    const searxngReady = backendReady
    const sqliteReady = backendReady
    const cloudReady = !!(aiStatus?.cloud_ready ?? true)
        ...
        <div className="page-container">
            ...
            <div className={styles.mainTabNav}>
                ...
                <Link href="/form-iso" className={styles.newAssessmentBtn}>
                    <Plus size={14} />
                    <span>{t('analytics.newAssessment') || 'Đánh giá mới'}</span>
                </Link>
            </div>
                ...
                <div>
                    ...
                    <section className={styles.section}>
                        <div className={styles.historyHeader}>
                            <p className="section-title" style={{ margin: 0 }}>{t('analytics.assessmentHistory')}</p>
                            <div className={styles.historyControls}>
                                <input
                                    type="text"
                                    placeholder={t('analytics.searchAssessmentsPlaceholder') || 'Tìm kiếm tổ chức hoặc mã ID...'}
                                    value={historySearch}
                                ...
                                >
                                    <option value="all">{t('analytics.filterAll') || 'Tất cả mức độ'}</option>
                                    <option value="compliant">{t('analytics.filterCompliant') || 'Đạt chuẩn (≥ 80%)'}</option>
                                    <option value="partial">{t('analytics.filterPartial') || 'Một phần (50% - 79%)'}</option>
                                    <option value="gap">{t('analytics.filterGap') || 'Cần cải thiện (< 50%)'}</option>
                                </select>
                            </div>
                        </div>
