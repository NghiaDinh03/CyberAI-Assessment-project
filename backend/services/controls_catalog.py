...
def get_control_groups(standard: str, custom_std: dict = None,
                       group_size: int = 6) -> list:
    """Split controls into smaller groups of *group_size* controls each.

    ``get_categories()`` returns large categories (e.g. A.5 has 37 controls).
    This function splits each category into sub-groups so each LLM prompt
    stays within the model's context window (~6000 tokens).

    Returns a list of dicts: ``[{"category": "A.5 Tổ chức (1/7)", "controls": [...]}, ...]``
    """
    categories = get_categories(standard, custom_std)
    groups = []
    for cat in categories:
        cat_name = cat.get("category", "Unknown")
        controls = cat.get("controls", [])
        if len(controls) <= group_size:
            groups.append({"category": cat_name, "controls": controls})
            continue
        n_chunks = max(1, (len(controls) + group_size - 1) // group_size)
        for i in range(0, len(controls), group_size):
            chunk = controls[i:i + group_size]
            chunk_num = (i // group_size) + 1
            groups.append({
                "category": f"{cat_name} ({chunk_num}/{n_chunks})",
                "controls": chunk,
            })
    return groups
